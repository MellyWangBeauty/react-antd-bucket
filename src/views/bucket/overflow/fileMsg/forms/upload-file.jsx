...
import {Button, Card, Dropdown, Form, Icon, Menu, Modal, Table, Tooltip, Upload, message} from "antd";
...
class AddUserForm extends Component {
    ...
    uploadFile = async (file, index) => {
        ...
        const initUploadParams = JSON.stringify({
            chunkSize: chunkSize,
            chunkNum: chunkCount,
            fileName: file.name, bucketName: this.props.bucket.bucketName, identifier: fileMd5, totalSize: fileSize
        })
        ...
        request({
            ...
            if (res.code === 200) {
                ...
                this.composeFile(res.data.uploadId, file.name, chunkCount, fileSize, file.contentType, fileMd5)
                return;
            }
            //
            // if (res.code === 0) {
     ...
     * @param fileName
     */
    composeFile = (uploadId, fileName, chunkSize, fileSize, contentType, identifier) => {
        ...
        const composeParams = JSON.stringify({
            ...
            expire: 12, maxGetCount: 2,
            bucketName: this.props.bucket.bucketName, identifier
        })
        $.ajax({
            ...
            type: 'POST', contentType: "application/json", processData: false, data: composeParams,
            success: function (res) {
    ...
    render() {
        ...
        const {fileList, uploading, ff, cut, showCropper, cropImageUrl} = this.state
        ...
        const t2 = (
            <span>
                ...
                <Button loading={uploading} style={{marginTop: 20 + 'px'}} type='primary'
                        onClick={this.uploadFiles.bind(this)}>上传文件</Button>

                 <Button style={{marginLeft: 20 + 'px'}} type='danger'
                         onClick={this.setFileList.bind(this, [])}>清空文件</Button>
            </span>
        )
        return (
            <div>
                ...
                >
                    <Card title={t2}>
                        <Table bordered rowKey="name" dataSource={fileList} pagination={false}>
                            <Column title="文件名" dataIndex="name" key="name" align="center" onCell={() => {
                                return {
                                    style: {
                                        maxWidth: 150,
                                        overflow: 'hidden',
                            ...
                            <Column title="操作" key="action" width={150} align="center" render={(text, row, index) => {
                                let filename = row.name; //文件路径地址
                                let index1 = filename.lastIndexOf(".");
                                let index2 = filename.length;
                                let postf = filename.substring(index1, index2); //获取文bai件后缀名duzhi
                                //判断文件后缀名是否等于视频文件的后缀名
                                const isVideo = postf === ".avi" || postf === ".mp4" || postf === ".rmvb";
                                let menu = "";
                                if (isVideo) {
                                    menu = (<Menu>
