  ...
  render() {
    ...
    const t2 = (
      <span>
        ...
        <Button type='primary' onClick={this.createAuth.bind(null)}>添加标签</Button>
          ...
          <div>
            ...
            <Card title={t2}>
              <Table bordered rowKey="id" dataSource={users} pagination={false} >
                ...
                <Column title="标签值" dataIndex="name" key="name" align="center" />
                <Column title="操作" key="action" width={150} align="center" render={(text, row) => (
                  <span>
                    <Link to="/spaceOverflow" style={{ marginRight: 10 + 'px' }}>保存</Link>
                    <Link to="/spaceOverflow" style={{ marginRight: 10 + 'px' }}>编辑</Link>
                    <Link to="/spaceOverflow" style={{ marginRight: 10 + 'px' }}>删除</Link>
                  </span>
                )} />
              </Table>
            </Card>
